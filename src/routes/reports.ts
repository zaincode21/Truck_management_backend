 ...
 */
router.get('/financial', authenticateUser, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (startDate || endDate) {
      where.delivery_date = {};
      if (startDate) {
        where.delivery_date.gte = new Date(startDate as string);
      }
      if (endDate) {
        where.delivery_date.lte = new Date(endDate as string);
      }
    }
    ...
    if (startDate || endDate) {
      doc.fontSize(10).text(
        `${startDate ? formatDate(startDate as string) : 'All Time'} - ${endDate ? formatDate(endDate as string) : 'Present'}`,
        { align: 'center' }
      );
    }
    doc.moveDown(1);
    ...
    doc.fontSize(8).fillColor('#666666').text(
      `Generated on ${new Date().toLocaleString()} | TruckFlow Logistics Management System`,
      { align: 'center' }
    );
